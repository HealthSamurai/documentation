...
document.addEventListener('keydown', function (event) {
  ...
  if (event.target.id === 'search-input') {
    ...
    if (dropdown && (dropdown.hasAttribute('data-search-dropdown') || dropdown.querySelector('a[data-search-result-index]'))) {
      ...
      console.log('Dropdown check:', dropdown, 'Results found:', results.length);
      ...
      if (results.length > 0 && (event.key === 'ArrowDown' || event.key === 'ArrowUp' || event.key === 'Enter')) {
        ...
        if (event.key === 'ArrowDown') {
          ...
          console.log('ArrowDown pressed, current index:', currentIndex, 'results length:', results.length);
          ...
          event.preventDefault();
          console.log('ArrowUp pressed, current index:', currentIndex, 'results length:', results.length);
          ...
          if (currentIndex >= 0 && currentIndex < results.length) {
            console.log('Enter pressed, clicking result at index:', currentIndex);
...
function updateSelectedResult(results, selectedIndex) {
  results.forEach((result, index) => {
    if (index === selectedIndex) {
      ...
      result.classList.remove('hover:bg-tint-2', 'hover:text-tint-strong', 'text-tint');
      ...
      const enterIcon = result.querySelector('div:last-child');
      if (enterIcon) {
