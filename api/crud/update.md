...
This is a more complex way to update a resource, but it gives more power. You can update a resource without knowing the `id` , but it requires the knowledge of [Search](../search/). Different response codes will be returned (based on the number of search results):
