...
(defn render-example-card
  ...
  [:div.border.border-tint-2.rounded-lg.p-6.hover:shadow-lg.transition-shadow.duration-200.bg-white
   ...
   [:div.flex.flex-wrap.gap-2
    (for [feature features] ; Show all features
...
(defn render-filter-checkbox
  ...
  [:label.flex.items-center.space-x-2.cursor-pointer.hover:bg-tint-1.px-2.py-1.rounded
   [:input.w-4.h-4.text-primary-9.border-tint-4.rounded.focus:ring-primary-5
    {:type "checkbox"
     :name type
     :value value
...
(defn render-search-bar []
  [:div.w-full
   [:form {:hx-get "/docs/aidbox/examples"
           :hx-target "#examples-content"
           :hx-trigger "keyup changed delay:300ms from:#examples-search, change from:.filter-checkbox"
           :hx-push-url "false"}
...
(defn render-examples-grid [examples]
  [:div#examples-grid.grid.grid-cols-1.md:grid-cols-2.lg:grid-cols-3.gap-6
   {:data-examples (json/write-str examples)} ; Store data for client-side filtering
   (if (seq examples)
     (for [example examples]
       ^{:key (:id example)}
       (render-example-card example))
     [:div.col-span-full.text-center.py-12
      [:p.text-tint-8 "No examples available"]])])
...
(defn filter-examples
  ...
  (let [search-lower (when search-term (str/lower-case search-term))]
    (filter (fn [example]
              (and
               ;; Search filter
               (or (empty? search-lower)
...
(defn render-examples-page
  ...
  [context]
  (let [;; Ensure we're in aidbox product context
        ctx-with-product (products/set-current-product-id context "aidbox")
        examples-data (indexer/get-examples ctx-with-product)
        {:keys [examples features_list languages_list timestamp]} examples-data]

    ;; Content only - layout will add navigation
    [:div.w-full.bg-tint-1.min-h-screen.py-8
     [:div.container.mx-auto.px-4.max-w-7xl
      ...
      [:div.mb-8
       ...
       ;; Main content: filters on left, examples on right
     ...
     [:script {:type "text/javascript"}
      ...
      (function() {
        ...
        const countElement = document.getElementById('examples-count');

        if (!examplesGrid) return;

        const allExamples = JSON.parse(examplesGrid.dataset.examples || '[]');
        console.log('Loaded examples:', allExamples);
        ...
        function filterExamples() {
          ...
          const hasActiveFilters = activeFilters.languages.length > 0 || activeFilters.features.length > 0;
          if (clearButton) {
          ...
          const filtered = allExamples.filter(example => {
            ...
            if (activeFilters.languages.length > 0) {
              const hasLanguage = example.languages && example.languages.some(lang =>
            ...
            if (activeFilters.features.length > 0) {
              const hasFeature = example.features && example.features.some(feature =>
          ...
          // Update count
          if (countElement) {
            countElement.textContent = `Showing ${filtered.length} of ${allExamples.length} examples`;
          }

          // Re-render filtered examples
          renderFilteredExamples(filtered);
        }

        function renderFilteredExamples(examples) {
          if (!examplesGrid) return;

          // Get all example cards
          const allCards = examplesGrid.querySelectorAll('.example-card');
          ...
          if (examples.length === 0) {
            ...
            let noResultsDiv = examplesGrid.querySelector('.no-results-message');
            if (!noResultsDiv) {
              noResultsDiv = document.createElement('div');
              noResultsDiv.className = 'col-span-full text-center py-12 no-results-message';
              noResultsDiv.innerHTML = '<p class=\"text-tint-8\">No examples match your filters</p>';
              examplesGrid.appendChild(noResultsDiv);
            }
            noResultsDiv.style.display = '';
          } else {
            // Hide no results message if it exists
            const noResultsDiv = examplesGrid.querySelector('.no-results-message');
            if (noResultsDiv) {
              noResultsDiv.style.display = 'none';
            }
            
            // Show/hide cards based on filter
            const visibleIds = new Set(examples.map(e => e.id));
            allCards.forEach(card => {
        ...
        checkboxes.forEach(cb => {
          cb.addEventListener('change', filterExamples);
        });
...
(defn examples-handler
  ...
  [context request]
  (let [uri (:uri request)
        content (render-examples-page context)

        ;; Custom layout without left navigation
        full-page [:div
    ...
    ;; Return response using gitbok.http/response1
    (http/response1
     (layout/document
      context
      full-page
