...
With parameters which start with [dot](broken-reference) you can filter resources by equality, e.g. `.name.0.family=<string>`
...
We can filter events by [dot expressions](broken-reference). Filtering Patient events by family name:
