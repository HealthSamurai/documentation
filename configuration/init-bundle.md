...
2. Note, that the [Aidbox format](https://docs.aidbox.app/storage/aidbox-and-fhir-formats) is not supported.
3. Aidbox handles an `id` in the body of the POST request. That's why posting the resource with an id twice will cause an `duplicate key` error. Use [conditional create](../api/crud/fhir-and-aidbox-crud.md#conditional-create) or [update](../api/crud/update.md) for that.
