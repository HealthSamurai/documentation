...
| `PGDATABASE`            | Name of the database that is used to store resources. `postgres` database name is not supported. Use `AIDBOX_AIDBOX_DB` environment variable if you really need to set `postgres` to be the database for Aidbox. |
...
| `AIDBOX_EXTENSION_SCHEMA`                                            | Schema for PostgreSQL extensions. Default is current schema. See [use different PostgreSQL schema section](configure-devbox-aidbox-multibox.md#use-different-postgresql-schema).                                                                                                                                                                                  |
...
| `BOX_FEATURES_ACL_CONDITIONAL__CRUD__ENABLE__CHECKS` | Enables ACL checks on conditional CRUD searches                                                                                                                                                                                                                                                                                                                                   |
...
| `AIDBOX_CLUSTER_DOMAIN` | <p>Used to route users between the Aidbox instances</p><p>For example: <code>aidbox.example.com</code></p>                                    |
| `AIDBOX_SUPERUSER`      | <p>Root user login and password separated by colon character (<code>:</code>) for Multibox</p><p>For example: <code>login:password</code></p> |
