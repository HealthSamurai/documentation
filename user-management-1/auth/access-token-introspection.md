...
If JWT is valid, Aidbox will put it's claims into the request object under `jwt` key, so you'll be able to access them with [AccessPolicy checks](../../security-and-access-control-1/security/access-control.md). If the token failed validation \(it's expired or signature isn't correct\) then the client will get a 401 "Unauthorised" response.
