...
A more complex way to create a resource (it requires the knowledge of [search](../fhir-search/)) but it gives some additional flexibility. If you provide search parameters, `create` becomes `conditional create` and works in the following way (depending on the number of search results):
