...
{% hint style="info" %}
Please note that Mapping returns [FHIR Transaction Bundle](../../../api/transaction.md), so it can produce as many CRUD operations as you need. Any other Aidbox operation/endpoint can be triggered as well (for instance, [SQL endpoint](../../../database/schema.md)).
{% endhint %}
