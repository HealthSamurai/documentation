...
{% hint style="info" %}
Please note that Mapping returns [FHIR Transaction Bundle](broken-reference), so it can produce as many CRUD operations as you need. Any other Aidbox operation/endpoint can be triggered as well (for instance, [SQL endpoint](broken-reference)).
{% endhint %}
